...
fs.readFile("input.txt", "utf8", (err, data) => {
	...
	for(var i = 0; i < distributions.length; i++) {
		if(compare(distributions[i],container)) {
			firstindex = i;
			break;
		}
	}
	console.log("The number of cycles needed to be completed since first encountering the current configuration is " + (distributions.length - firstindex) + ".");
});