		...
		for(let i = 0; i < permuted[l].length; i++) {
			for(let k = 0; k < distancesArr.length; k++) {
				if(i + 1 != permuted[l].length) {
					if(((distancesArr[k]["from"] == permuted[l][i]["value"]) && (distancesArr[k]["to"] == permuted[l][i + 1]["value"])) || (distancesArr[k]["to"] == permuted[l][i]["value"]) && (distancesArr[k]["from"] == permuted[l][i + 1]["value"])) {
...
fs.readFile("input.txt", "utf8", (err, data) => {
	...
	possibilities.sort();
	console.log("The distance of the shortest route is " + possibilities[0] + ".");
});