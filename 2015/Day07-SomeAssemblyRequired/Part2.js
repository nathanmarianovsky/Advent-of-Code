...
fs.readFile("input.txt", "utf8", (err, data) => {
	...
	let	holder = allConnections(collection),
		arr = [];
	...
	for(let key in arr) {
		if(arr[key]["wire"] == "a") {
			holder = allConnections(collection);
			for(let iter in holder) {
				if(holder[iter]["wire"] == "b") { holder[iter]["value"] = arr[key]["value"]; }
			}
		}
	}
	arr = [];
	...
	for(let key in arr) {
		if(arr[key]["wire"] == "a") { console.log("The new signal provided to wire 'a' is " + arr[key]["value"] + "."); }
	}
});