...
var turn = (player, boss, attacker, effects, magic) => {
	var damage = 0,
		outcome = [];
	// if(attacker == "player") { player.hit_points--; }
	
	var arr = effects.reverse();
	...
	// 	}
	// 	iter.num--;
	// 	if(boss.hit_points < 0) { break; }
	// });

	for(var k = 0; k < arr.length; k++) {
		if(arr[k].num == -1 && arr[k].name == "Shield") { player.armor = 0; }
		else if(arr[k].num == -1 && arr[k].name != "Shield") {
			activate_effect(player, boss, arr[k]);
		}
		if(arr[k].num > -1) { 
			activate_effect(player, boss, arr[k]);
		}
		arr[k].num--;
		if(boss.hit_points < 0 || player.hit_points <= 0) { break; }
	}

	// if(boss.hit_points > 0 && player.hit_points > 0) {
		if(attacker == "player") {
			player.hit_points--;
...
var battle = (player, boss, moves) => {
	...
	while(player.hit_points > 0 && boss.hit_points > 0 && (count / 2) < moves.length) {
		...
		if(count % 2 == 0) {
			...
			// player.hit_points--;
			// console.log(player);
			// if(player.hit_points > 0) { turn(player, boss, "player", effects, moves[count / 2]); }
			turn(player, boss, "player", effects, moves[count / 2]);
			// console.log(player);
		}
		else { turn(player, boss, "boss", effects); }
		count++;
	}
};
				...
				container.forEach(cur => {
					...
					for(var k = 0; k < scenario.length; k++) {
						...
						if(scenario[k] == "Shield") {
							scenario.length - k >= 3 ? tmp = scenario.slice(k + 1, k + 3) : tmp = scenario.slice(k + 1);
...
fs.readFile("input.txt", "utf8", (err, data) => {
	...
	holder.forEach(elem => {
		if(elem.status == "WON") { result.push(elem); }
	});
